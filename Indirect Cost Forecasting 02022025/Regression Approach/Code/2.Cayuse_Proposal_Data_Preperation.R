...
# Load Proposal Data
proposal_data <- read_csv(paste0(cayuse_input_path, "Proposal_Data_All_06032025.csv"))
...
college_name_data_Ashlee <- read_excel(paste0(ashlee_input_path, "College_Admin_Unit_Mapping.xlsx"))
...
reduced_pattern <- regex("^\\s*Reduced F&A by Sponsor", ignore_case = TRUE)

proposal_data <- proposal_data %>% 
  ## 1. Convert blanks ("", " ") to proper NA
  mutate(across(c(`indirect cost rate on`, `indirect cost rate off`),
                ~ na_if(trimws(.x), ""))) %>% 
  
  ## 2. Build the new 3-category variable
  mutate(
    Reduced_FA = case_when(
      # both columns missing  → keep NA
      is.na(`indirect cost rate on`) & is.na(`indirect cost rate off`)   ~ NA_character_,
      
      # “Reduced …” phrase found in the ON-campus column
      str_detect(`indirect cost rate on`,  reduced_pattern)              ~ "Reduced Indirect Cost On Campus",
      
      # otherwise, phrase found in the OFF-campus column
      str_detect(`indirect cost rate off`, reduced_pattern)              ~ "Reduced Indirect Cost Off Campus",
...
approval_cols <- c(
  "irb approval",
  "iacuc approval",
  "bioagents toxins approvals",
  "infectious agents approvals",
  "controlled substances approval",
  "radioactive required approvals"
)

proposal_data <- proposal_data %>%
  # Normalize blanks and casing
  mutate(across(all_of(approval_cols),
                ~ na_if(str_to_lower(trimws(.x)), ""))) %>%
  
  # Create 2-level Approval_Required variable
  mutate(
    Approval_Required = if_else(
      rowSums(across(all_of(approval_cols), ~ .x %in% c("approved", "pending")), na.rm = TRUE) > 0,
      "Yes",
      "Unknown"
    )
  )
...
table(proposal_data$Approval_Required)




foreign_cols <- c("foreign collaboration", "international travel", "foreign shipment")

proposal_data <- proposal_data %>%
  # Normalize the 3 foreign-related columns (handle blanks, casing)
  mutate(across(all_of(foreign_cols),
                ~ str_to_lower(trimws(.x)))) %>%
  
  # Create Foreign_Involvement flag
  mutate(
    Foreign_Involvement = if_else(
      rowSums(across(all_of(foreign_cols), ~ .x == "yes", .names = NULL), na.rm = TRUE) > 0,
      "Yes",
      "No"
    )
  )
...
proposal_data <- proposal_data %>%
  # Normalize casing and trim spaces for both columns
  mutate(across(c(`chemical agents`, `hazardous materials`),
                ~ str_to_lower(trimws(.x)))) %>%
  
  # Create indicator variable
  mutate(
    Chemical_or_Hazard = if_else(
      `chemical agents` == "yes" | `hazardous materials` == "yes",
      "Yes",
      "No"
    )
  )
...
# technology-related or intellectual property (IP)
...
proposal_data <- proposal_data %>%
  # Clean all 4 columns (lowercase + trim)
  mutate(across(all_of(tech_ip_cols),
                ~ str_to_lower(trimws(.x)))) %>%
  
  # Create binary indicator
  mutate(
    Technology_or_IP_Involved = if_else(
      rowSums(across(all_of(tech_ip_cols), ~ .x == "yes"), na.rm = TRUE) > 0,
      "Yes",
      "No"
    )
  )
